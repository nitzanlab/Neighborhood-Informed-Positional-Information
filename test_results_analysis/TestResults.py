...
class TestResults(ABC):
    def __init__(self, test_results, test_type, train_mean, train_std, decoding_genes, save=False, savedir='',
                 edge_trim=False):
    ...
    def plot_decoding_map(self, title='', vmax=10, is_wt=True, xlim=None):
        ...
        if is_wt:
            plt.plot(range(self.raw_test_results.shape[1]), range(self.raw_test_results.shape[1]), color='red',
                     linestyle='--', linewidth=3, label='ground truth')
            plt.legend()
    ...
    @classmethod
    def from_pickle(cls, savedir, test_type, decode_genes):
