...
def get_cov(training_data):
    ...
    for pos in range(num_positions):
        ...
        # alternate covariance calculation
        dat = training_data[:, pos, :]
        dat = dat - np.mean(dat, axis=0)[None]
        covs[pos] = dat.T@dat/(dat.shape[0]-1)
...
def plot_all_gasturloid_genes_exp_together():
    ...
    for gene, expr in all_genes_exp_dict.items():
        ...
        plt.plot(x, (mean_expr), color=GASTRULOID_GENE_COLORS[gene], linewidth=2)
        plt.fill_between(x, (mean_expr - std_expr), (mean_expr + std_expr), color=GASTRULOID_GENE_COLORS[gene], alpha=0.5,label=gene)

    plt.xlabel(POSITION_X_LABEL)
    plt.ylabel('gene expression')
    # plt.title("Gastruloid Gene Expression")
    plt.legend()
    # yticks = np.arange(0.0, 1, 5)
    # plt.yticks(yticks, [f"{v:.1f}" for v in yticks])
    plt.xlim(0.1,0.9)
    plt.ylim(0, 1)
    plt.tight_layout()
    plt.savefig('figures/gastruloids_exppr.pdf')
    plt.savefig('figures/gastruloids_exppr.png')
    plt.show()
    plt.close('all')
...
def pos_error_one_gene(gene_name):
    ...
    cov_wn = get_cov(gene_wn_exp)
    sc_pos_err = calculate_position_error_gt_pos(cov_sc[:,np.newaxis,np.newaxis], mean_sc[:,np.newaxis])
...
def get_pos_error_all_two_genes_combos(to_plot=True):
    ...
    return np.array(all_two_gene_wn_err), np.array(all_two_gene_sc_err)
...
def get_pos_error_all_three_gene_combos(to_plot=True):
    ...
    all_wn_pos_error_3genes = np.array([wn_pos_err_ind, wn_pos_err_dep1, wn_pos_err_dep2, wn_pos_err_dep3])
    all_sc_pos_error_3genes = np.array([sc_pos_err_ind,sc_pos_err_dep1,sc_pos_err_dep2,sc_pos_err_dep3])
...
def get_all_subsets_pos_error(to_plot=True):
    ...
    print(f'gastruloids NI singe gene: {np.mean(np.nanmedian(wn_pos_error_1gene, axis=1))}')
    print(f'gastruloids CI singe gene: {np.mean(np.nanmedian(sc_pos_error_1gene, axis=1))}')
    # ##two genes (6)
    ...
    wn_arr = [wn_pos_error_1gene, wn_pos_error_2genes,wn_pos_error_3genes, wn_pos_4]
    sc_arr = [sc_pos_error_1gene, sc_pos_error_2genes, sc_pos_error_3genes, sc_pos_4]
    plot_pos_error_mean_num_genes_gastruloids(wn_arr, sc_arr)

def compute_mean_std(arrays, start_idx, end_idx):
    stats = []
    for arr in arrays:
        sliced = arr[:, start_idx:end_idx]
        row_means = np.nanmedian(sliced, axis=1)  # mean per row ignoring NaNs
...
def plot_pos_error_mean_num_genes_gastruloids(wn_arr, sc_arr):
    ...
    means_wn = [m for m, s in stats_wn]
    stds_wn = [s for m, s in stats_wn]
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig('figures/gastruloids_preds.pdf')
    plt.savefig('figures/gastruloids_preds.png')
    plt.show()
...
def get_pos_error_all_one_gene(to_plot):
    ...
    return np.array(all_wn_pos_error_1gene), np.array(all_sc_pos_error_1gene)
...
def create_cov_and_mean_joint_datasets_wn():
    ...
    full_wn_covs = np.zeros((sox2_wn_cov.shape[0], 12, 12))

    # diagonal of covariance
    full_wn_covs[:, :3, :3] = sox2_wn_cov
    full_wn_covs[:, 3:6, 3:6] = bra_wn_cov
    full_wn_covs[:, 6:9, 6:9] = cdx2_wn_cov
    full_wn_covs[:, 9:, 9:] = foxc1_wn_cov
    # off diagonals
    # full_wn_covs[:, 3:6, :3] = bra2_sox2_wn_cov
    # full_wn_covs[:, :3, 3:6] = bra2_sox2_wn_cov.transpose((0, 2, 1))
    # full_wn_covs[:, 6:9, :3] = cdx2_sox2_wn_cov
    # full_wn_covs[:, :3, 6:9] = cdx2_sox2_wn_cov.transpose((0, 2, 1))
    # full_wn_covs[:, 9:, :3] = foxc1_sox2_wn_cov
    # full_wn_covs[:, :3, 9:] = foxc1_sox2_wn_cov.transpose((0, 2, 1))
...
def create_covariance_sc_joint_datasets():
    ...
    # replace NaNs with 0s (just in case)
    bra2_sox2_cov = np.nan_to_num(bra2_sox2_cov, nan=0)
    cdx2_sox2_cov = np.nan_to_num(cdx2_sox2_cov, nan=0)
    foxc1_sox2_cov = np.nan_to_num(foxc1_sox2_cov, nan=0)
...
def calculate_position_error_gt_pos(full_covs, full_means):
    ...
    for pos in range(num_pos):
        ...
        variance = np.sum(mean_exp_slopes[pos, :]*np.linalg.solve(full_covs[pos, :, :], mean_exp_slopes[pos, :]))
        position_error[pos] = 1/variance
        # position_error[pos] = 1 / (mean_exp_slopes[pos, :] @ np.linalg.inv(
...
def plot_positional_information_gastruloids():
    ...
    i_unique = np.log2(GASTRULOID_N)*np.ones_like(i_sc)
    i_unique_min = np.log2(GASTRULOID_N_MIN)*np.ones_like(i_sc)
    i_unique_max = np.log2(GASTRULOID_N_MAX)*np.ones_like(i_sc)
    ...
    plt.tight_layout()
    plt.savefig('figures/gastruloids_posinfo.pdf')
    plt.savefig('figures/gastruloids_posinfo.png')
    plt.show()
...
def pos_err_all_four_genes(to_plot=True):
    ...
    print(f'gastruloids NI pos-err: {np.mean(wn_pos_error):.3f} +- {np.std(wn_pos_error):.3f}, quants {quants(wn_pos_error):.3f}')
    print(f'gastruloids CI pos-err: {np.mean(sc_pos_error):.3f} +- {np.std(sc_pos_error):.3f}, quants {quants(sc_pos_error):.3f}')
    return wn_pos_error, sc_pos_error
